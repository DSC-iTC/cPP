...
+
_If asymmetric key generation in accordance with FCS_CKM.1/AKG is selected, the selection-based SFR FCS_CKM.1/AK must be claimed by the TOE._
+
_If symmetric key generation in accordance with FCS_CKM.1/SKG is selected, the selection-based SFR FCS_CKM.1/SK must be claimed by the TOE._
...
|NIST SP 800-133r2 (Section 6.1) with ISO 18031:2011 as an approved RBG in addition to those in NIST SP 800-133r2 (Section 4).
...
+
_The selection of key size 512 bits is for the case of XTS-AES using AES-256. In the case of XTS-AES for both AES-128 and AES-256, the developer is expected to ensure that the full key is generated using direct generation from the RBG as specified in NIST SP 800-133r2 (Section 6.1)._
+
