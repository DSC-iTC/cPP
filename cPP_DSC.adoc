...
FDP_SDC_EXT.1.1:: The TSF shall use [.underline]#[*selection*: protected storage, symmetric encryption using [*selection*: AES-CCM, AES-GCM, AES-CBC, CAM-CBC, CAM-CCM, CAM-GCM] as specified in FCS_COP.1/SKC, key encapsulation using [*selection*: KAS1, KTS-OAEP, RSAES-PKCS1-v1_5] as specified in FCS_COP.1/KeyEncap, key wrapping using [*selection*: KW, KWP] as specified in FCS_COP.1/KeyWrap]# to protect the confidentiality of authorization data and [*_assignment*: list of internally and externally stored SDEs identified in the Confidential SDE List attribute of an SDO_].
...
FPT_PHP.3.1:: The TSF shall resist [_data extraction via fault injection, extreme temperatures, abnormal voltage_] to the {empty}[_TSF storage elements that contain [.underline]#[selection: SDEs, SDOs, firmware]#_] by responding automatically such that the SFRs are always enforced.
