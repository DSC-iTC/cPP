...
Users: The entities using the DSC will be client applications on the platform. They may be acting as proxies for users or may have identities of their own. The DSC will not be able to distinguish the difference; therefore, the cPP will recognize an entity known as the Client Application (CApp), as the user presenting authentication tokens and authorization values (collectively known as authorization data) to the DSC for the purposes of identity verification and authorization to perform operations. <<Roles>> discusses the concept of users in more detail. This cPP also recognizes a special user called the administrator, which typically has access to DSC objects normally denied to CApps (see definition of objects below).
...
A platform leveraging DSCs as a hardware-secured Key Store facilitates the use of secure and protected storage of symmetric and asymmetric keys for access to data and services. These DSCs would ensure the confidentiality and integrity of the secret and private keys, as well as the integrity of public keys, inside the protected hardware boundary.
