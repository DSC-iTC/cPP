...
|Parse
|The DSC shall ingest SDEs (e.g., pre-installed keys, credentials, tokens) and attributes from trusted components or services external to its boundary either across a secured channel or in a manner that the objects are protected for use only by the DSC.
