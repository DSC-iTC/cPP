...
!FIA_HMR_EXT.1 !Online Authentication Hammering Prevention
...
FIA_HMR_EXT.1.1:: The TSF shall limit the number of authentication attempts for each counter specified in FIA_AFL_EXT.1.1 by [.underline]#[selection: using _[assgnment: integer number greater than or equal to 1,000 iterations]_ of PBKDF as specified in FCS_CKM_EXT.8.1, specifying a maximum of _[assignment: number of attempts]_ that can occur duing _[assignment: time period]_, setting the minimum time period between consecutive attempts to _[assignment: time period]_, preventing future attempts for a time period as defined in FIA_AFL_EXT.1.3, _[assignment: other anti-hammering method]_]#.

_Application Note {counter:remark_count}_:: _The authentication throttling applies to every authentication mechanism. The attempts may be counted according to FIA_AFL_EXT.1.1 per each counter that is available. The developer can implement the timing of the delays in the requirements using unequal or equal timing of delays._
...
|FIA_HMR_EXT.1
|This requirement defines how excessive authentication attempts are minimized/mitigated to prevent online hammering attacks.
...
==== FIA_HMR_EXT Online Hammering Prevention
*Family Behavior*
This family defines requirements for the TOE's behavior related to repeated authentication attempts.
*Component Leveling*
[#img-FIA_HMR_EXT] 
[ditaa,"FIA_HMR_EXT"]
....
    ...
    +--------------------------------------------+ 
    |                                            |     +---+
    | FIA_HMR_EXT Online Hammering Prevention    +---->| 1 |
    |                                            |     +---+
    +--------------------------------------------+ 
....
FIA_HMR_EXT.1 Online Authentication Hammering Prevention, requires the TSF to limit the number of possible attempts allowed.
*Management: FIA_HMR_EXT.1*
The following actions could be considered for the management functions in FMT:
* None
*Audit: FIA_HMR_EXT.1*
There are no auditable events foreseen.
...
FCS_CKM.6 
|FCS_COP.1/KeyedHash and FCS_CKM.6 are required by the PP. FDP_ITC_EXT.1 and FDP_ITC_EXT.2 are also required by the PP and equivalent to FDP_ITC.1 in this case because they all relate to a mechanism by which key data can be imported into the TSF.
