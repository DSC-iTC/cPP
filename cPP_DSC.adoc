...
+
_The dependency on FCS_OTV_EXT.1 is only used for signature schemes that require random bits, such as ECDSA, KCDSA, or EC-KCDSA._
+
...
The security functional requirements FDP_ACC.1 Subset Access Control and FDP_ACF.1 Security Attribute Based Access Control have been selected to ensure the TOE resists attacks targeting the release of SDOs or SDEs to unauthorized users. The corresponding security functional policy addresses the relationship between the subjects, objects and operations within the context of the TSF.

Access control in the TOE is controlled by the interactions between the subjects, objects and operations. In general for the TOE, the expectation would be that the majority of subject-object-operation combinations would be prohibited by the TSF, and that this would be the default mode of operation. The default mode of operation is critical for the undestanding of the access control rules, as the majority of the rules will be exceptions to the default. In the case of a default blocking control, the TSF subject-object-operation combinations that allow access, either always or under specific conditions are the primary focus since these grant access to the SDOs and SDEs.

The parameters for prohibited actions within the TOE may include anything, such as the object not being able to be modified, or because there is a specific rule prohibiting the actions. The access control security functional requirements provide a way to explain the subject-object-operation interactions.
...
_Application Note {counter:remark_count}_:: _FDP_ACF.1.3 and FDP_ACF.1.4 allow for optional override conditions to resolve otherwise contradictory Access Control SFP rules. For example, the rule "S.Admin may always modify the SDO.Conf attribute of any OB.P_SDO or OB.T_SDO object" may be overridden by a statement in FDP_ACF.1.4 that identifies any particular SDO objects as non-modifiable regardless of subject authorizations._
+
_The DSC may contain pre-installed SDOs. The DSC enforces access control for pre-installed SDOs like any other SDO it contains or manages._
