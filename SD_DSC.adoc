...
The test EAs for individual SFRs identify cases where developer tools may be
needed to execute the test as written.
...
If the ST author has selected [.underline]#common application developer#, this test is performed by either using applications from different developers or appropriately (according to API documentation) not authorizing sharing. 
...
==== Confidentiality of SDEs (Extended - FDP_SDC_EXT)
...
In this case, use the evaluation activities of the FPT_PHP.3 if [.underline]#protected storage# is selected, FCS_COP.1/SK if [.underline]#symmetric encryption using…# is selected, and FCS_COP.1/KAT if [.underline]#key wrapping using…# is selected.
...
. instructions to establish all penetration test prerequisite initialconditions;
...
|The CEM work unit is captured in <<Reporting>>; there are no substantive differences.
...
. A list of flaw hypotheses that are generated through the use of iTC-defined tools (e.g. nmap, protocol testers) and their application is specified in <<Type 4 Hypotheses—Tool-Generated>>.
...
[loweralpha]
. Search Common Vulnerabilities and Exposures: https://cve.mitre.org/cve/[https://cve.mitre.org/cve/]
. Search the National Vulnerability Database: https://nvd.nist.gov/[https://nvd.nist.gov/]
. Search US-CERT https://www.kb.cert.org/vuls/search/[https://www.kb.cert.org/vuls/search/]
