...
*Test 1:* [Conditional: Test applies to keys and key material where the caller-visible key reference is reused after a key destruction.]

. Record the value or a corresponding key check value (KCV) of the key or keying material. The key check value is an obfuscated value of the key, for example by hashing, encrypting or wrapping the key.
. Cause the TOE to destroy the key.
. Record the value or a corresponding key check value (KCV) of the key or keying material and compare it with the value from step 1. If both are identical, the test fails.

*Test 2:* [Conditional: Test applies to keys and key material where the caller-visible key reference is not reused after a key destruction.]

. Cause the TOE to destroy the key.
. Attempt to invoke a cryptographic service with use the caller-visible key reference and verify that the service is unavailable.
