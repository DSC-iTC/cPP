...
The developer shall provide sufficient information to the evaluator to properly define the implementation of the algorithm. The evaluator shall define test groups according to the implementation of the algorithm, for each combination of the following parameters:
* Hash algorithm

For each test group, the evaluator shall generate at least 6 test cases meeting the following requirements:
...
For each test group, the evaluator shall also generate at least one Monte Carlo Test (MCT) test case, consisting of an arbitrary seed and a list of 100 digest values. For hash algorithms, the MCT is defined as follows:
----
for i = 1 through 100
    msg = seed
...
* Group size (KCDSA)
* Private key size (LMS, XMSS)
* Padding scheme (RSA)
* Hash or XOF algorithm

For each test group, the evaluator shall generate at least 6 test cases meeting the following requirements:

* Each test case shall consist of an arbitrary private key (as defined by the test group) and an arbitrary input message to be signed.
...
* Public key size (LMS, XMSS)
* Padding scheme (RSA)
* Hash or XOF algorithm

For each test group, the evaluator shall generate at least 6 test cases meeting the following requirements:

* Each test case shall consist of an arbitrary public key (as defined by the test group), an arbitrary input message, and a signature.
* A representative sample of the domain of supported message sizes shall be tested.
* For 5 test cases, the public key or input message is modified such that the signature is invalid.
* For the remaining test case, the signature shall be valid.
...
For each test group corresponding to a CBC, CFB, or OFB mode of operation, the evaluator shall also generate at least one Monte Carlo Test (MCT) test case, consisting of an arbitrary key, an arbitrary initialization vector, an arbitrary input plaintext/ciphertext, and a list of 100 corresponding ciphertexts/plaintexts.
