...
The tests are based on having a representative set of test vectors that exercise the algorithm. Within a single algorithm implementation there are many possible configuration parameters for how the algorithm is implemented. Some examples of these parameters can include:
...
The evaluator shall generate a set of test vectors to exercise the algorithm according to the specified implementation details. Any parameters with a range shall test, at least, the minimum and maximum values specified for the parameters. The test vectors shall test each possible combination of the parameters. For example, if the algorithm has 2 optional input variables, a range of input lengths and 1 output length, then for each input length that is tested, each configuration of the optional input variables shall be tested (if the variables can be used together, that is another combination to be tested).
