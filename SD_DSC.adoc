...
[conditional] If the key is generated according to a symmetric key scheme, the evaluator shall review the TSS to determine that it describes how the functionality described by FCS_CKM.1/SKG is invoked. The evaluator uses the description of the RBG functionality in FCS_RBG.1, or the key derivation functionality in either FCS_CKM.5 or FCS_CKM_EXT.8, depending on the key generation method claimed, to determine that the key size being requested is greater than or equal to the key size and mode to be used for the encryption/decryption of the data.
...
The algorithm tests require the developer to provide access to a test platform that provides the evaluator with tools that are typically not found on factory products.

The developer shall provide sufficient information to the evaluator to properly define the implementation of the algorithm. 

All test results shall be compared to a known-good implementation to ensure the correct result is produced by the TSF. The evaluator shall determine the proper known-good implementation and provide a rationale for its use for testing. Internal or external implementations are acceptable with a sufficient rationale.

The tests are based on having a representative set of test vectors that exercise the algorithm. Within a single algorithm implementation there are many possible configuration parameters for how the algorithm is implemented. Some examples of these parameters can include (this is not an exhaustive list):

* input length
* optional input variables 
* output length
* bit vs byte orientation
* key generation method
* role
* hash algorithm

The evaluator shall generate a set of test vectors to exercise the algorithm according to the specified implementation details. The set of test vectors shall follow the following guidelines:

* Any parameters with a range shall test, at least, the minimum and maximum values specified for the parameters. 
* Each combination of variables (such as optional inputs) should be tested
* A representative set of up to 20 test groups (at least 10, depending on the complexity of the algorithm) with random valid combinations of parameters shall be created. Each test group will contain 10 test vectors.
* [conditional] If the algorithm has any known edge-cases that are considered insecure (such as input parameters that should be explicitly excluded from use), the test shall include verification of the known edge-cases.

The evaluator shall be able to provide a rationale as to the sufficiency of the test vector set in exercising the algorithm.
...
For each selected curve, and for each key pair generation method as described in FIPS 186-4, section B.4, the evaluator shall require the implementation under test to generate 10 private/public key pairs (d, Q). The private key, d, shall be generated using a random bit generator as specified in FCS_RBG.1. The private key, d, is used to compute the public key, Q'. The evaluator shall confirm that 0\<d\<n (where n is the order of the group), and the computed value Q' is then compared to the generated public/private key pairs' public key, Q, to confirm that Q is equal to Q'.
...
===== FCS_CKM_EXT.8 Cryptographic Operation (Password-Based Key Derivation Functions)
